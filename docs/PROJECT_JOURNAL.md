...
**Next up on return: ACCORDION.** Composer builds the rack per `SI2_accordion_map.md` §3 (new track "Accordion SI2", new loopMIDI port `Accordion`, 10 slots A1–A10 incl. `_Classical KS Wheel`); survey protocol pre-designed in §4 — star test = AC2 bellows test (CC1-driven dynamics on the Wheel patch: timbre-changing crescendo through held notes, the accordion's native language). Sandbox config already carries the accordion entry.
