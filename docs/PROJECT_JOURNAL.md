...
**Open at session end:** *(mid-session checkpoint, 2026-08-03, composer on short break)* —
**Next up on return: patch-by-patch listening battery** across the 16 harp techniques (same 6-test battery as §5c; spec-before-sound format; scope-tag every finding: engine/group/patch). Continuous-rich patches likely most interesting (Buzzing Pedal, Glissando modes, Bisbigliando).
State: Keystation 88 working in G1 after three real bugs (MIDIIN2 control-port binding, statechange feedback loop → frozen tab, focused-button SPACE un-arming record — all in SAMPLER_QUIRKS/commits). SB3 Technique Explorer live: 16-slot rack configured & committed in the .rpp (slot=channel), technique buttons + loop playback + phrase save/load (`sandbox/motives/`, first two phrases saved). Capture strip built but ON HOLD (composer taxonomy pending). CC probe battery on Ordinario done → deep map §5c (CC7 volume lane ✓, bend ±2st ✓, CC120 secco ✓, CC64 laissez-vibrer ✓, CC11/CC1 null). G5 (reusable parameterized effect units) logged as palette germ. Rack to-do: IR Reverb OFF on all 16 parts. Probe #10: [Param] list enumeration.
