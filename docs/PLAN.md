...
- **S4 — Sandbox v1** — `doing` *(pulled forward 2026-08-02 in v0 form, fused with the minimum of S3's MIDI plumbing; first use case = S2.3a harp dynamics→timbre)* — → 1c.
  ...
  - **The Gesture Loop** (capture → annotate → render → audition; composer vision in COMPOSER_LOG 2026-08-02; the Motive Builder germ — data format = identity + compiled rendering, D7 in miniature):
    ...
    - **G3 — Span effects** — `deferred → composition time` *(composer MO, 2026-08-04)* — select a group of notes → apply effect across the span: crescendo first (rendering decision settled by CC probes: velocity ramps for note-streams, CC7 envelopes for sampled events/sustaining instruments), bend contours. Built when composing demands it.
    - **G5 — Reusable effect units** — `deferred → composition time` *(composer MO, 2026-08-04; the palette germ)* — configured span effect saved as a named, parameterized unit; applied from a menu, re-parameterized per use. Unit parameters = future object handles.
    - **Standing AI watch** *(composer, 2026-08-04)*: flag architecture that must be right BEFORE composition-time tool building (data-model shapes, capture formats, switching abstraction) — get ahead of forced redesigns. Current watch list: motive JSON carries annotations array ✓ · rendering-recipe switching abstraction (channel/KS/CC0) not yet coded · timbre-preset save format undecided.
    - **G4 — Save/load named motives** — `todo` — gesture+annotations JSON → motive library begins.
    - *Later layers (architecture-ready, not scheduled):* timing→notated-rhythm analysis · **playability fact-checker** (on-demand: button/prompt evaluates a phrase against performer-reality constraints per instrument — constraints data accumulates in the deep maps; early version can be AI reading the motive JSON).
  - **SB3 — Technique Explorer** — `todo` *(composer 2026-08-03: likely next)* — all of an instrument's techniques reachable + instantly flippable while playing live or looping a phrase; capture strip (band + comment → TIMBRE_VOCAB) built in, so exploring IS documenting. Doubles as the **switching-mechanism probe**: channel-per-technique vs keyswitches vs CC/PC (Xsample-style) — the rapid-per-note-switching architecture decision falls out of real use (M1). Rendering recipes then declare each instrument's mechanism; the compiler abstracts it.
  - **SB4 — Continuous-control lab** — `todo` — curves/LFO-ish interface (kin to the score's curves) for patches with continuous axes (CC1 expression, bend, later Xsample CC world); starts as a CC1 slider, grows shapes. Presets savable like phrases (timbre/gesture presets — assignable to notes in G2+).
